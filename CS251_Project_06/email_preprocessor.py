...
def count_words(email_path='data/enron'):
    ...
    # Descend into the dataset base directory
        ...
        for filename in filenames:
            ...
            with open(file_path, 'r', encoding='utf-8', errors='ignore') as f:
                if("txt" in filename):
                    ...
                    # Use the `tokenize_words` function to chunk it into a list of words
                    words = tokenize_words(email)
...
def find_top_words(word_freq, num_features=200):
    ...
    top_words = []
    counts = []
    if(len(word_freq) < num_features):
        num_features = len(word_freq)
    for i in range(num_features):
        max_key_value_pair = max(word_freq.items(), key=lambda item: item[1])
        top_words.append(max_key_value_pair[0])
        counts.append(word_freq[max_key_value_pair[0]])
        word_freq[max_key_value_pair[0]] = 0
...
def make_feature_vectors(top_words, num_emails, email_path='data/enron'):
    ...
    # Initialize empty arrays to store feature vectors and class indices
    feats = np.zeros((num_emails, len(top_words)), dtype=np.int32)
    y = np.zeros(num_emails, dtype=np.int32)

    counter = 0

    # Iterate over each class folder (spam/ham) in the email dataset base folder
    for class_idx, class_folder in enumerate(['spam', 'ham']):
        class_path = os.path.join(email_path, class_folder)
        emails = os.listdir(class_path)

        # Iterate over each individual email in the class folder
        for i, email in enumerate(emails):
            email_file = os.path.join(class_path, email)

            # Read the email file as a string
            with open(email_file, 'r', encoding='utf-8', errors='ignore') as f:
                email_str = f.read()

            # Tokenize the email string into a list of words
            words = tokenize_words(email_str)


            # Count the occurrence of each word in the top_words list
            word_counts = {}
            for word in words:
                # print(word)
            ...
            # Set the feature vector for the email
            for j, word in enumerate(top_words):
                feats[counter, j] = word_counts.get(word, 0)
            counter += 1

            # Set the class index for the email
            y[i] = class_idx
...
def retrieve_emails(inds, email_path='data/enron'):
    ...
    emails = []
    for ind in inds:
        email_file = os.path.join(email_path, f'email{ind}.txt')
