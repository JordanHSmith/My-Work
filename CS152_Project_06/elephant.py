...
def calcSurvival(param_list, pop_list):
    ...
    for e in pop_list:
        ...
        elif(e[IDXAge] <= param_list[IDXMaximumAge]):
...
def defaultParameters():
    '''
    No parameters
    Returns a list of the parameters for the simulation to run
    '''
    ...
    nyears=200
    par_list = [calvint,probdart,juvage,maxage,probcalfsurvival,probadultsurvival,probseniorsurvival,carryingcapacity,nyears]
...
def elephantSim(percDart,inputParameters = None):
    '''
    (float,list) -> ()
    Calculates the average elephant population after 5 simulations.
        It then determines if the population is too big or too small.
    '''
    ...
    for i in range(len(results)):
        total_pop += results[i][0]
    average_pop = total_pop / len(results)

    return(int(parameters[IDXCarryingCapacity] - average_pop))
...
def main(argv):
    ...
    The function is intended to print out the average number of the various
        groups of elephants, depending upon what the input probability of darting is.
    '''
