...
class PCA_COV:
    ...
    def covariance_matrix(self, data):
        ...
        cov = (1 / (data.shape[0]-1)) * (c_data.T @ c_data)
        return cov
    ...
    def compute_cum_var(self, prop_var):
        ...
        for i in range(len(prop_var)):
            if(i==0):
                props.append(prop_var[i])
            else:
                props.append(prop_var[i] + props[i-1])
        return props
    ...
    def pca(self, vars, normalize=False):
        arr = np.array(self.data[vars])
        self.orig_ranges = []
        self.orig_mins = []
        for i in range(arr.shape[1]):
            self.orig_ranges.append(np.max(arr[:,i])-np.min(arr[:,i]))
            self.orig_mins.append(np.min(arr[:,i]))
        ...
        if(normalize):
            cov = self.covariance_matrix(arr)
        else:
            cov = self.covariance_matrix(self.data[vars])
    ...
    def elbow_plot(self, num_pcs_to_keep=None):
        if(num_pcs_to_keep == None):
            ...
            for i in range(self.A.shape[1]):
                indices.append(i)
            plt.plot(indices,self.cum_var)
        else:
            indices = []
            vecs = []
            for i in range(num_pcs_to_keep):
                indices.append(i)
                vecs.append(self.e_vecs[i])
            plt.plot(indices,self.cum_var[:num_pcs_to_keep])
        
        plt.title("Principle Parts vs. Cumulative Variance")
        plt.xlabel("Number of Principle Parts")
    ...
    def pca_project(self, pcs_to_keep):
        e_vecs = np.ones((self.e_vecs.shape[0],len(pcs_to_keep)))
        index = 0

        for i in range(len(self.e_vecs)):
            if(i in pcs_to_keep):
                e_vecs[:,index] = self.e_vecs[:,i]
                index += 1

        self.A_proj = self.A.dot(e_vecs)
        return self.A_proj
    ...
    def pca_then_project_back(self, top_k):
        P_sub = self.e_vecs[:, :top_k]
        A_proj_sub = self.A_proj[:, :top_k]
        reconstructed_data = A_proj_sub.dot(P_sub.T)
