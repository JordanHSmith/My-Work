...
class Analysis:
    ...
    def min(self, headers, rows=[]):
        ...
        # indices = self.data.get_mappings()[headers]
        # print(indices)


        # if(rows != []):
        #     return np.amin(self.data[:rows[0]],axis = 0)
        # else:
    ...
    def pair_plot(self, data_vars, fig_sz=(12, 12), title=''):
        ...
        fig,ax = plt.subplots(nrows = len(data_vars), ncols = len(data_vars),figsize = fig_sz, sharex=True,sharey=True)
        print(data_vars)
        ...
        for i in range(len(data_vars)):
            ...
            for j in range(len(data_vars)):
                ...
                ax[i,j].scatter(data_copy[:,self.data.get_mappings()[data_vars[i]]],data_copy[:,self.data.get_mappings()[data_vars[j]]])
